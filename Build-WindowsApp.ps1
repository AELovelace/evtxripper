...
try {
    ...
    if ($Target -in @('gui', 'both')) {
        Invoke-PyInstallerBuild -PythonExe $pythonExe -Root $repoRoot -EntryScript 'chainsaw_gui.py' -AppName 'EVTXRipperGUI' -Windowed $true -DoClean:$Clean -BundleEnv:$IncludeEnv
    ...
    if ($Target -in @('tui', 'both')) {
        Invoke-PyInstallerBuild -PythonExe $pythonExe -Root $repoRoot -EntryScript 'chainsaw_frontend.py' -AppName 'EVTXRipperTUI' -Windowed $false -DoClean:$Clean -BundleEnv:$IncludeEnv
